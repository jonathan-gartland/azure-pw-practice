...
test.describe("Example to demonstrate testing of simple and nested iframes in Playwright", () => {
  test("Simple iframe - Input text in the text editor which is inside an iframe", async ({
    ...
    const textarea = page.frameLocator("#mce_0_ifr").locator("#tinymce");
    // await textarea.fill('Testersdock.com')
    await expect(textarea).toHaveText("Your content goes here.");
  });
